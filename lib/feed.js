...
*/
Feed.prototype.list = function(parameters, callback, optinalDataFormat) {
  var options       = makeFeedOptions('GET', '', parameters);
  var dataFormat    = optinalDataFormat || defaultDataFormat;
  request(this.key, options, callback, dataFormat);
};
...
*  @param {string}   optinalDataFormat Optional data dataFormat parameter, json or xml, default to json.
*/
Feed.prototype.create = function(dataFile, callback, optionalDataFormat) {
  var options         = makeFeedOptions('POST');
  var dataFormat      = optinalDataFormat || defaultDataFormat;
  request(this.key, options, callback, dataFormat , dataFile);
};
...
Feed.prototype.read = function(id, parameters, callback, optionalDataFormat ) {
  ...
  require(this.key, options, callback, dataFormat);
};
...
* @param {function} callback          The callback function.
* @param {string}   optinalDataFormat Optional data dataFormat parameter, json or xml, default json.
*/ 
Feed.prototype.update = function(id, dataFile, callback, optionalDataFormat ) {
  id = id.toSource();
  var options         = makeFeedOptions('PUT', id, parameters);
  var dataFormat      = optionalDataFormat  || defaultDataFormat;
  request(this.key, options, callback, dataFormat, dataFile);
};
...
Feed.prototype.delete = function(id) {
  ...
  request(this.key, options, callback);
};
